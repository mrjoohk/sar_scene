...
class SARSimulator:
    ...
    def backprojection_image(self, signal, nx=500, ny=500):
        ...
        index = 0
        for xyz in sensor_xyz: # per pulse

            # Calculate the range profile
            range_profile = self.xp.fft.fftshift(self.xp.fft.ifft(signal_gpu[index, :], self.fft_length)) # 펄스마다 RC가 병목
            ...
            range_image = self.xp.sqrt((xyz[0] - x_image) ** 2 + (xyz[1] - y_image) ** 2 + (xyz[2] - z_image) ** 2) - r0[index] # 병목
            ...
            bp_image += f(range_image) * self.xp.exp(term * range_image) # 병목
